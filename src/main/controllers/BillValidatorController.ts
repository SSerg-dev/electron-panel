...
class BillValidatorController extends EventEmitter {
	...
	public enabler = async (enable: boolean = false) => {
		...
		if ( this.state === States.DISCONNECTED || status == 0x15 || status == 0x17 ) {
			return
		}
		if ( !enable ) {
	...
	private setListeners = async () => {
		...
		this.device.on('cassette-out-of-position', () => log(TAG, "Cassette out of position"))
		...
		this.device.on('failure', (msg: string, code: string) => log(TAG, msg, code))
		/* ----------------------------------------------------------------------- */
		/*  */
		this.device.on('rejecting', (msg: string, code: string) => log(TAG, msg, code))
		/* ----------------------------------------------------------------------- */
		/*  */
