...
class OPCUAService extends EventEmitter {
  constructor(url) {
    ...
    this.client.on("connection_failed", () => {
			...
			log(TAG, `Client connection retry attempt ${attemptNumber} retrying in ${delay}ms.`)
		}).on("close", () => {
  ...
  async start(type = 'wash', num = 1) {
    ...
    try {
			...
			this.session.on("keepalive", () => {
				...
				log(TAG, `Session encountered a keepalive error: ${state !== undefined ? state.toString() : "No state provided."}`)
      ...
      this.subscription = await this.session.createSubscription2({
			...
			this.subscription.on("started", async () => {
				log(TAG, `Subscription started, subscriptionId: ${this.subscription.subscriptionId}`)
      }).on("terminated", () => {
				log(TAG, `Subscription '${this.subscription.subscriptionId}' was terminated.`)
			}).on("keepalive", () => {
				log(TAG, `Subscription '${this.subscription.subscriptionId}' keepalive received.`)
			}).on("internal_error", (err) => {
				log(TAG, `Subscription '${this.subscription.subscriptionId}' encountered an internal error: ${err.message}`)
				...
				log(TAG, `Subscription '${this.subscription.subscriptionId}' encountered a (non-internal) error: ${err.message}`)
			}) 
      ...
      for (let tag in nodes) {
        ...
        itemsToMonitor.push({
          nodeId: "ns=" + TAG_NS + ";s=" + nodes[tag].replace('{0}', String(num - 1)),
      ...
      for (let index = 1; index < maxUsersNumber + 1; index++) {
        for (let tag in userNodes) {
          itemsToMonitor.push({
            nodeId: "ns=" + TAG_NS + ";s=" + userNodes[tag].replace('{0}', String(index)),
            attributeId: AttributeIds.Value
          })
        }
      }
