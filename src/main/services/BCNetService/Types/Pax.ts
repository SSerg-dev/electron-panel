...
import { ADR_CARD_READER, ADR_BILL_VALIDATOR, CRC_POLY } from '../Constants'
  ...
  open = () => {
    ...
    return new Promise<any>((resolve, reject) => {
      if (self.serial.isOpen) {
        ...
        console.log('++self.serial.open-->', self.serial)
      ...
      if (
        ...
        switch (status[0]) {
          ...
          case 0x1c:
            ...
            else if (status[1] == 0x93)
              code = 'Incorrect number of characters in barcode'
            ...
            this.emit(
              'rejecting',
              `Rejecting due to ${code}`,
              status[1].toString(16).padStart(2, '0')
            )
            break
