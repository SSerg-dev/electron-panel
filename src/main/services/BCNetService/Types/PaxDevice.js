...
class PaxDevice extends EventEmitter {
   ...
   */
  constructor(port, currency, bills, debug = true) {
    super()
    let self = this
    this.status = ''
    this.terminalType = TERMINAL_TYPE || 'VM00080951'
    this.dataLength = 0
   ...
   * On serial events
   */
  onSerialPortOpen() { return this.debug('Serialport Open.') }
  onSerialPortError(error) { return this.debug('Serialport Error:', error) }
  onSerialPortClose() { return this.debug('Serialport Close.') }
  ...
  // end read
  // ------------------------------------
  async readNextResponse(request) {
    let self = this
    let res = 42
    self.counter
  ...
  // ------------------------------------
  getStatus(response) {
    const index = response.findIndex((s) => s === SUCCESS)
    return response[index + 1].toString(16)
  }
  sendSuccessAmount(amount, status) {
  ...
  toHex(num) { return Math.abs(num).toString(16) }
  ...
  getRequest() {
    ...
    this.paxRequest.messages.forEach((item, index) => {
