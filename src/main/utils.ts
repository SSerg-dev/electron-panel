...
export const log = function(from: any = '', text: any = '', data: any = '') {
    ...
    const sData = data ? (typeof(data) === "object" ? JSON.stringify(data) : data) : ''
    console.log(
...
export const setIPToLocalSubnet = (num : number | string): string => getLocalIP().split('.').slice(0,-1).concat(String(num)).join(".")
